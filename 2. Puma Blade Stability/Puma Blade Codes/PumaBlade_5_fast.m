function [stability, damping_box] = PumaBlade_5_fast(link_stiff, cg_off, S)
    No = 2;
    
    %% 1. STRUCTURAL RECONSTRUCTION
    % Calculate scalar offset based on percentage input
    ...
    %% 4. Flutter Resolution (With Dynamic Integrals)
    tolerancia = 10e-5;
    Amatrix = zeros(2*No, 2*No); Amatrix(1:No, (No+1):end) = eye(No);
    tol = 100; counter1 = 0; k_box = zeros(1, No); damping_box = zeros(1, No);
    
    % Preallocate dynamic aero matrices
    WW_K = zeros(No); WT_K = zeros(No); TW_K = zeros(No); TT_K = zeros(No);
    ...
    for modes=1:No
        while tol>tolerancia
            % Vectorial Theodorsen function (varies along the span)
            k = 4/3*Autovalores(modes)*b/S.D.omega./S.xaero;
            ...
            Kaero = Bw*Cw; Caero1 = Ca; Caero2 = Bw*Chatw; Maero = Ma;
            
            % ONLY integrate the matrices affected by Ck inside the loop
            for i=1:No
                for j=1:No
                    WW_K(i, j) = trapz(S.xaero, psi(:, i).*psi(:, j).*Uvec.^2.*Ck);
                    WT_K(i, j) = trapz(S.xaero, psi(:, i).*phi(:, j).*Uvec.^2.*Ck);
                    TW_K(i, j) = trapz(S.xaero, phi(:, i).*psi(:, j).*Uvec.^2.*Ck);
                    TT_K(i, j) = trapz(S.xaero, phi(:, i).*phi(:, j).*Uvec.^2.*Ck); 
                    
                    WW_Ck(i, j) = trapz(S.xaero, psi(:, i).*psi(:, j).*Uvec.*Ck);
                    WT_Ck_Theta(i, j) = trapz(S.xaero, psi(:, i).*phi(:, j).*Uvec.*Ck);
                    WT_Ck_Omega(i, j) = trapz(S.xaero, S.D.omega*Uvec.*psi(:, i).*psi_p(:, j).*Ck); 
                    TW_Ck(i, j) = trapz(S.xaero, phi(:, i).*psi(:, j).*Uvec.*Ck); 
                    TT_Ck_Theta(i, j) = trapz(S.xaero, phi(:, i).*phi(:, j).*Uvec.*Ck);
                    TT_Ck_Omega(i, j) = trapz(S.xaero, S.D.omega*Uvec.*phi(:, i).*psi_p(:, j).*Ck); 
                end
            end
            
            KK = -Kaero(1,1)*WW_K + Caero1(1,2)*WT_C_Omega + Caero2(1,2)*WT_Ck_Omega + Kaero(1,2)*WT_K ...
                 -Kaero(2,1)*TW_K + Caero1(2,2)*TT_C_Omega + Caero2(2,2)*TT_Ck_Omega + Kaero(2,2)*TT_K;
                 
            CC = -Caero1(1,1)*WW_C - Caero2(1,1)*WW_Ck + Caero1(1,2)*WT_C_Theta + Caero2(1,2)*WT_Ck_Theta ...
                 -Caero1(2,1)*TW_C - Caero2(2,1)*TW_Ck + Caero1(2,2)*TT_C_Theta + Caero2(2,2)*TT_Ck_Theta;
            ...
            % Tracking of the eigenvector -> maximum scalar product
            ...
            % Redefinition of eigenvectors for next iteration
