function [stability, damping_box] = PumaBlade_4_fast(link_stiff, cg_off, S)
    No = 10;
    ...
    % Keep 10 modes for this section
    Autovectores = Autovectores(:, 1:No);
    Autovalores = Autovalores(1:No);
    ...
    % Generalized Mass Matrix (Identity assumption)
    M_gen = eye(No);
    
    % Generalized Stiffness Matrix (Forced pure diagonal)
    K_gen = Autovectores' * K * Autovectores;
    K_gen = diag(diag(K_gen));
    ...
    for modes=1:No
        ...
        while tol>tolerancia
            ...
            % Redefinition of eigenvectors for next convergence iteration
